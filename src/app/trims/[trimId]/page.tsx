...
import {
  ...
  EXTERIOR_IMAGES,
  INTERIOR_IMAGES,
} from "@/components/car/details/Design";
...
import { getRandom, INTERIOR_LABELS, formatCurrency } from "@/constants/trims";
        ...
        {VARIANTS.map((v) => (
          ...
          >
            <div className="text-left">
              <p
                className={`text-sm font-semibold ${currentVariant === v.name ? "text-blue-400" : "text-white"}`}
              >
                {v.name}
              </p>
...
export default function CarDetailsPage({
  ...
  useEffect(() => {
    if (!trimData || !modelImagesData) return;

    const price =
      typeof trimData.price === "number"
        ? trimData.price
        : ((trimData.price as any)?.exShowroom ?? 0);

    const priceLabel = formatCurrency(price);

    // Using backend metadata (label and isPrimary) as requested
    const apiImages = (modelImagesData as any)?.results || [];
    const sortedApiImages = [...apiImages].sort(
      (a: any, b: any) => (a.order || 99) - (b.order || 99),
    );

    const interiorFromApi = sortedApiImages
      .filter((img) => INTERIOR_LABELS.includes(img.label?.toLowerCase() || ""))
      .map((img) => img.url);

    const exteriorFromApi = sortedApiImages
      .filter(
        (img) => !INTERIOR_LABELS.includes(img.label?.toLowerCase() || ""),
      )
      .map((img) => img.url);

    const primaryImage = sortedApiImages.find(
      (img) => img.isPrimary || img.label?.toLowerCase() === "hero",
    )?.url;
    ...
    setCar({
      ...
      description:
        ...
        `${trimData.brand?.name} ${trimData.model?.name} with automatic transmission in premium finish. This luxury vehicle redefines versatility with refined performance and cutting-edge technology.`,
      safetyRatings: {
        globalNcap: {
          adult: (trimData.rating || 4) >= 4.5 ? 5 : 4,
          child: (trimData.rating || 4) >= 4.5 ? 5 : 3,
        },
        bharatNcap: {
          status: Math.random() > 0.3 ? "Not Tested" : "Tested",
          adult: 5,
          child: 4,
        },
      },
      ...
      insights: [
        ...getRandom(
          [
            "Advanced safety package (6 airbags, ADAS Level 2)",
            "Improved interior design and materials",
          ],
          2,
        ).map((label) => ({ type: "positive" as InsightType, label })),
        ...getRandom(
          ["Neutral: No idea about servicing and maintenance"],
          1,
        ).map((label) => ({ type: "neutral" as InsightType, label })),
        ...getRandom(["Customer service at authorised dealerships"], 1).map(
          (label) => ({ type: "negative" as InsightType, label }),
        ),
      ],
      riInsights: {
        overallRating: Number((3 + Math.random() * 2).toFixed(1)),
        recommendationPercent: 70 + Math.floor(Math.random() * 25),
        mostMentioned: ["Mileage", "Comfort", "Power", "Style"][
        ...
        categoryRatings: [
          {
            label: "Build Quality & Safety",
            subLabel: "Build Quality & Paint Issues",
            score: Number((3.5 + Math.random() * 1.5).toFixed(1)),
          },
          {
            label: "Features & Tech",
            subLabel: "Missing basic features",
            score: Number((3.5 + Math.random() * 1.5).toFixed(1)),
          },
          {
            label: "After Sales Service",
            subLabel: "Need improvements - High Impact",
            score: Number((2 + Math.random() * 2.5).toFixed(1)),
          },
          {
            label: "Engine Reliability",
            subLabel: "Mixed long term feedback",
            score: Number((3.5 + Math.random() * 1.5).toFixed(1)),
          },
          {
            label: "Performance",
            subLabel: "Below segment expectations",
            score: Number((3.5 + Math.random() * 1.5).toFixed(1)),
          },
          {
            label: "Comfort & Interiors",
            subLabel: "Spacious and Comfortable",
            score: Number((4 + Math.random()).toFixed(1)),
          },
        ],
        userFeedback: {
          positives: [
      ...
      gallery: {
        exterior:
          exteriorFromApi.length > 0
            ? exteriorFromApi
            : EXTERIOR_IMAGES.map((i) => i.src),
        interior:
          interiorFromApi.length > 0
            ? interiorFromApi
            : INTERIOR_IMAGES.map((i) => i.src),
      },
    });
  ...
  const ringImages = [...car.gallery.exterior, ...car.gallery.interior].slice(
    0,
    8,
  );
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground pb-24">
      ...
      <nav className="absolute top-0 left-0 right-0 h-14 z-50 flex items-center justify-between px-6 bg-transparent pointer-events-none">
        ...
        <div className="flex items-center gap-2 pointer-events-auto">
          <Button
            variant="ghost"
            size="icon"
            className="text-white/80 hover:text-red-400 hover:bg-white/10 rounded-full transition-colors"
          >
          ...
          <Button
            variant="ghost"
            size="icon"
            className="text-white/80 hover:text-blue-400 hover:bg-white/10 rounded-full transition-colors"
          >
      ...
      {/* Hero */}
      <section
        className="pt-14 pb-8 bg-[#0F172A] text-white relative overflow-hidden shadow-xl"
        ...
        <div className="relative z-10 container mx-auto px-4 flex flex-col items-center">
          ...
          <div className="mt-5 flex flex-col items-center gap-1 relative">
            ...
            >
              ...
              <ChevronDown
                className={`h-3 w-3 transition-transform duration-200 ${variantOpen ? "rotate-180" : ""}`}
              />
            </button>
            <VariantsDropdown
              isOpen={variantOpen}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <Card
                key={s.label}
                className="p-4 bg-card border-none shadow-sm text-center"
              >
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart
                data={[
                  {
                    quarter: "Q1",
                    sales: SALES_DATA.slice(0, 3).reduce(
                      (a, b) => a + b.sales,
                      0,
                    ),
                  },
                  {
                    quarter: "Q2",
                    sales: SALES_DATA.slice(3, 6).reduce(
                      (a, b) => a + b.sales,
                      0,
                    ),
                  },
                  {
                    quarter: "Q3",
                    sales: SALES_DATA.slice(6, 9).reduce(
                      (a, b) => a + b.sales,
                      0,
                    ),
                  },
                  {
                    quarter: "Q4",
                    sales: SALES_DATA.slice(9, 12).reduce(
                      (a, b) => a + b.sales,
                      0,
                    ),
                  },
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-card border-t border-border p-4 md:px-8 z-40 pb-6">
        <div className="container mx-auto max-w-5xl flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <Button
            size="lg"
            className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 rounded-xl h-12 font-semibold"
          >
            Book Now
          </Button>
        </div>
